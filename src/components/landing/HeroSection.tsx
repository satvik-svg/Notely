...
function FloatingCard({ children, className, delay = 0, rotate = 0 }: { children: React.ReactNode, className: string, delay?: number, rotate?: number }) {
  return (
    <motion.div
      ...
      className={`bg-card rounded-2xl shadow-float p-4 border border-border ${className}`}
    >
      {children}
    </motion.div>
  );
...
export function HeroSection() {
  return (
    <section
      className="relative min-h-[95vh] flex items-center justify-center overflow-hidden bg-background pt-20 pb-20"
    >
      ...
      <FloatingCard
        ...
        rotate={2}
      >
        <p className="text-xs font-display font-semibold text-foreground/90 mb-2">Recent uploads</p>
        <div className="space-y-2">
          {["DBMS Unit 3", "OS Memory Mgmt", "CN TCP/IP"].map((note) => (
            <div key={note} className="flex items-center gap-2">
              <div className="w-2 h-2 bg-brand-400 rounded-full" />
              <span className="text-xs text-muted-foreground font-body">{note}</span>
            </div>
          ))}
        </div>
      </FloatingCard>
      ...
      <FloatingCard
        ...
        rotate={2}
      >
        <p className="text-xs text-muted-foreground font-body mb-1">Top contributor</p>
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-brand-100 rounded-full flex items-center justify-center">
            <span className="text-xs font-display font-bold text-brand-600">SK</span>
          </div>
          <div>
            <p className="text-xs font-display font-semibold text-card-foreground">Satvik Kumar</p>
      ...
      <FloatingCard
        ...
        rotate={-2}
      >
        <p className="text-xs font-display font-semibold text-foreground/90 mb-2">Active groups</p>
        <div className="flex -space-x-1">
          ...
          <div className="w-7 h-7 bg-secondary rounded-full border-2 border-white flex items-center justify-center">
            <span className="text-muted-foreground text-[9px] font-body">+12</span>
          </div>
        </div>
      </FloatingCard>
      ...
      <div className="text-center z-10 px-6 max-w-4xl mx-auto flex flex-col items-center">
        ...
        >
          <div className="mb-8 p-3 bg-card rounded-2xl shadow-float-md inline-block">
          ...
          <h1 className="font-display text-5xl md:text-7xl lg:text-8xl font-bold text-foreground leading-[1.05] tracking-tight mb-4 max-w-3xl">
            Think, plan, and track <br/>
            <span className="text-muted-foreground/50">all in one place</span>
          </h1>

          <p className="font-body text-muted-foreground md:text-lg mb-8 max-w-lg mx-auto leading-relaxed mt-4">
