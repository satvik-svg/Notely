  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-card overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-50 bg-slate-50/50">
        ...
        <h3 className="font-display font-semibold text-slate-800 text-sm">Ask the notes</h3>
        <span className="ml-auto text-[10px] font-body bg-brand-50 text-brand-600 px-2 py-0.5 rounded-md">AI powered</span>
      </div>
      ...
      <div className="px-5 py-4 space-y-3 min-h-[180px] max-h-80 overflow-y-auto">
        {messages.length === 0 && (
          <div className="text-center py-8">
            <p className="text-xs font-body text-slate-400">Ask anything about this note</p>
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex gap-2 ${m.role === "user" ? "justify-end" : ""}`}>
            ...
            <div className={`max-w-[80%] text-xs font-body leading-relaxed px-3 py-2 rounded-xl
              ${m.role === "user"
      ...
      <div className="px-5 py-3 border-t border-slate-50 flex gap-2">
        <input value={input} onChange={(e) => setInput(e.target.value)}
          ...
          className="flex-1 text-xs font-body bg-slate-50 rounded-xl px-3 py-2.5 border border-slate-100 
                     outline-none focus:border-brand-300 focus:ring-1 focus:ring-brand-100 placeholder:text-slate-400"
        />
